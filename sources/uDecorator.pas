...
const
  Decors: array [0..99] of TDecoratorRec = (
    (
    SpriteID  : 0;
    Light     : 0;
    DecorSurf : dsFloor;
...
begin
  if (Map.Cell[Y][X].Tile in FloorSet)
    and (Map.Cell[Y][X].Decor < 0)
    and ((Decors[ID].DecorSurf = dsFloor)
    or (Decors[ID].DecorSurf = dsBoth)) then
      Map.Cell[Y][X].Decor := ID;
end;
...
begin
  Result := (I >= 0)
    and ((Map.Cell[Y][X].Tile in FloorSet)
    and ((Decors[I].DecorSurf = dsFloor)
    or (Decors[I].DecorSurf = dsBoth))
    or (Map.Cell[Y][X].Tile in [tlWall])
    and ((Decors[I].DecorSurf = dsWall)
    or (Decors[I].DecorSurf = dsBoth)))    
end;
...
begin
  try
    ...
    on E: Exception do uError.Error.Add('Decorators.Render', E.Message);
  end;
end;
...
begin
  Result := dtNone;
  if (S = 'dtNone') or (S = '') then Exit;
  if (S = 'dtBlood') then Result := dtBlood;
  if (S = 'dtStone') then Result := dtStone;
  if (S = 'dtSlime') then Result := dtSlime;
  if (S = 'dtBone')  then Result := dtBone;
  if (S = 'dtTrap')  then Result := dtTrap;
  if (S = 'dtWeb')   then Result := dtWeb;
end;
...
begin
  Result := 'dtNone';
  case DT of
    dtBlood: Result := 'dtBlood';
    dtStone: Result := 'dtStone';
    dtSlime: Result := 'dtSlime';
    dtBone : Result := 'dtBone';
    dtTrap : Result := 'dtTrap';
    dtWeb  : Result := 'dtWeb';
  end;
end;
