...
type
  TIconEnum = (icMale, icFemale, icPlus, icMinus, icQuestion, icGold, icFlag, icLife, icMana, icStar, icHammer, icElixir, icShield, icFood, icLeaf,
    icFeather, icStr, icDex, icBook, icVision, icFire, icIce, icLightning, icDrop, icSword, icSun, icMoon, icTroph, icAst, icKey, icRepair, icBook2);

type
  UI = class(TObject)
    class procedure Bar(const X, LM, Y, Wd: UInt; const Cur, Max: UInt; const AColor, ADarkColor: Cardinal);
    ...
    class function MenuItem(AKey, AStr: string; IsItem: Boolean = False): string;
  end;
...
const
  IconStr: array [TIconEnum] of string = ('F8D0', 'F8D1', 'F8D2', 'F8D3', 'F8D4', 'F8D5', 'F8D6', 'F8D7', 'F8D8', 'F8D9', 'F8DA', 'F8DB', 'F8DC',
    'F8DD', 'F8DE', 'F8DF', 'F8E0', 'F8E1', 'F8E2', 'F8E3', 'F8E4', 'F8E5', 'F8E6', 'F8E7', 'F8E8', 'F8E9', 'F8EA', 'F8EB', 'F8EC', 'F8ED',
    'F8EE', 'F8EF');
  F = '[[%s]]';

  { UI }

class procedure UI.Bar(const X, LM, Y, Wd: UInt; const Cur, Max: UInt; const AColor, ADarkColor: Cardinal);
var
  I, L, W: UInt;
begin
  L := Wd;
...
begin
  if (AColor = '') then
    ...
    Result := Format('[font=icon][color=%s][U+%s][/color][/font]', [LowerCase(AColor), UpperCase(IconStr[AIcon])]);
end;

class function UI.KeyToStr(AKey, AStr, AColor: string): string;
begin
  Result := Trim(Terminal.Colorize(Format(F, [UpperCase(AKey)]), Terminal.GetColorFromIni(AColor)) + ' ' + AStr);
end;

class function UI.MenuItem(AKey, AStr: string; IsItem: Boolean): string;
begin
  Result := IfThen(IsItem, UI.KeyToStr(AKey, '', 'Equip'), UI.KeyToStr(AKey)) + ' ' +
    Terminal.Colorize(AStr, IfThen(IsItem, Terminal.GetColorFromIni('Equip'), 'vision'));
end;
...
begin
  ...
  Terminal.Print(Terminal.Window.Width div 2, AY, Format(FT, [S]), TK_ALIGN_CENTER);
