...
begin
  ...
  // Life
  V := Game.EnsureRange(IfThen(MobBase[TMobEnum(ID)].Boss, (MobBase[TMobEnum(ID)].Level + Ord(Game.Difficulty)) * 25, 0), UIntMax);
  ...
  begin
    ...
    Attributes.SetValue(atPV, Math.EnsureRange(Math.RandomRange(Attributes.Attrib[atPV].Value,
      Attributes.Attrib[atPV].Value + (MobBase[TMobEnum(ID)].Level * Ord(Game.Difficulty))), Attributes.Attrib[atPV].Value, PVMax - 10));
  end;
end;
