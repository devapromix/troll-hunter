...
begin
  ...
  UI.Bar(1, 0, Y + (D * 2), W, ACur, AMax, color_from_name(LowerCase(ABarColor)),
    clDarkGray);
  Terminal.Print(X, Y + (D * 2), Format('%s %d/%d',
    [AIcons + ' ' + AStr, ACur, AMax]), TK_ALIGN_CENTER);
  Inc(D);
end;
...
begin
  ...
  if Mode.Wizard then
    Terminal.Print(X, Y + (D * 2), Format('%s %d(%d)/%d',
      [AIcons + ' ' + AStr, ACur.Value, ACur.Prm, AMax]), TK_ALIGN_CENTER)
  else
    Terminal.Print(X, Y + (D * 2), Format('%s %d/%d',
      [AIcons + ' ' + AStr, ACur.Value, AMax]), TK_ALIGN_CENTER);
  Inc(D);
end;

procedure TScenePlayer.RenderPlayer;
begin
  Terminal.Print(X, Y + 2, Format(FT, ['Attributes' + ' (1/2)']),
    TK_ALIGN_CENTER);
  // Level
  Add(Format('%s %d', ['Level', Player.Attributes.Attrib[atLev].Value]),
...
begin
  ...
  B := A * 3;
  Terminal.Print(B, Y, Format(FT,
    [Format('Skills (%d-%d)', [FSkillCursorTop + 1, FSkillCursorTop + ScrMax])]),
    TK_ALIGN_CENTER);
